...
# secscan_all_in_one_v3_pro_taint.sh — SAST one-shot with Semgrep Pro + taint (v3-pro)
# Focus: Semgrep only; enable --pro --auto (taint) with graceful fallback
# Outputs: SARIF + Top10 CSV + full CSV + summary.md + minimal HTML
# Usage: ./secscan_all_in_one_v3_pro_taint.sh . --jobs 6 --fail-on HIGH,CRITICAL --include '**' --metrics on
...
SCAN_ARGS+=( --sarif -o "/src/${SARIF}" --error --verbose )
...
RUN1_ARGS=( "${SCAN_ARGS[@]}" )
if [[ $PRO -eq 1 ]]; then RUN1_ARGS=( --pro "${RUN1_ARGS[@]}" ); fi

echo "[RUN] Semgrep (attempt 1) — Pro=${PRO}, Auto=${AUTO}" | tee -a "${LOG_FILE}"
set +e
"${DOCKER_RUN[@]}" "${SEMG_IMG}" "${RUN1_ARGS[@]}" |& tee -a "${LOG_FILE}"
rc=$?
set -e
...
if [[ $rc -ne 0 ]]; then
  ...
  RUN2_ARGS=( "${SCAN_ARGS[@]}" )
  set +e
  "${DOCKER_RUN[@]}" "${SEMG_IMG}" "${RUN2_ARGS[@]}" |& tee -a "${LOG_FILE}"
  rc=$?
  set -e
  if [[ $rc -ne 0 ]]; then
...
for run in d.get("runs",[]):
    ...
    for r in run in run.get("results",[]):
...
with open(all_csv,"w",newline="",encoding="utf-8") as f:
    import csv as _csv
    w=_csv.DictWriter(f,fieldnames=["Severity","Rule","Rule ID","File","Line","Message"]); w.writeheader(); w.writerows(rows)
...
with open(top10_csv,"w",newline="",encoding="utf-8") as f:
    _csv.writer(f).writerows(rows_top)
...
if [[ -n "${FAIL_ON}" ]]; then
  ...
  if [[ -s "${ALLCSV}" ]]; then
    ...
    while IFS= read -r sev; do
      C=$( (tail -n +2 "${ALLCSV}" | cut -d',' -f1 | grep -c -E "^${sev}$") || true )
      echo "  - ${sev}: ${C}"
      TOTAL=$((TOTAL + C))
    done <<< "${FAIL_SEVS}"
    if [[ $TOTAL -gt 0 ]]; then
